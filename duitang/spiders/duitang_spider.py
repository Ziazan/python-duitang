...
class DuiTangSpider(scrapy.Spider):
    ...
    # 第一个请求会从start_urls发起
    start_urls = [
        "https://www.duitang.com/napi/blog/list/by_filter_id/?include_fields=top_comments,is_root,source_link,item,buyable,root_id,status,like_count,sender,album,reply_count&filter_id=%s&start=%s&_=1585905865134" %(CATE_NAME,page)
    ]
    item = DuiTangItem()
    def parse(self, response):
        ...
        if(res['status'] == 1):
            ...
            if(self.next_start < self.MAX_CATCH_PAGES):
                url = "https://www.duitang.com/napi/blog/list/by_filter_id/?include_fields=top_comments,is_root,source_link,item,buyable,root_id,status,like_count,sender,album,reply_count&filter_id=%s&start=%s&_=1585905865134" %(self.CATE_NAME,self.next_start)
